...
# 卸载服务
uninstall_service() {
    log_step "开始卸载 Node Exporter..."
    
    # 停止服务
    if systemctl is-active --quiet node_exporter; then
        log_info "停止 Node Exporter 服务..."
        systemctl stop node_exporter
    fi
    
    # 禁用服务
    if systemctl is-enabled --quiet node_exporter; then
        log_info "禁用 Node Exporter 服务..."
...
show_menu() {
    echo -e "${GREEN}"
    echo "=================================="
    echo "  Node Exporter 管理脚本 v3.0"
    echo "=================================="
    echo -e "${NC}"
    
    if check_service_installed; then
        echo "1. 重新安装 Node Exporter"
        ...
        case $CHOICE in
            1)
                echo -n "检测到已安装 Node Exporter，是否继续安装？(y/N): "
                read -r CONFIRM <&3
                if [[ ! $CONFIRM =~ ^[Yy]$ ]]; then
                    log_info "已取消安装"
                    exit 0
                ...
                ;;
            2)
                echo -n "确定要卸载 Node Exporter 吗？(y/N): "
                read -r CONFIRM <&3
                if [[ $CONFIRM =~ ^[Yy]$ ]]; then
                    uninstall_service
                else
